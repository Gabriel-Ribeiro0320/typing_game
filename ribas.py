...
while running:
        ...
        if event.type == pygame.QUIT:
            ...
            if game_status == initial_menu:
                if event.key == pygame.K_SPACE:
                    game_status = time_setting
                    input_active = True
            elif game_status == time_setting:
                if event.key == pygame.K_RETURN:
                    if practice_time.isdigit():
                        game_status = game
                        game_duration = int(practice_time) * 1000
                        start_time = pygame.time.get_ticks()
                        input_active = False
                    else:
                        practice_time = ""
                elif event.key == pygame.K_BACKSPACE:
                    practice_time = practice_time[:-1]
                else:
                    if input_active and len(practice_time) < 3:
                        practice_time += event.unicode
    ...
    if game_status == game:
        ...
        time_remaining = max((game_duration - elapsed_time) // 1000, 0)
    ...
    if game_status == initial_menu:
        ...
        screen.fill(BLACK)
        text1 = font1.render("SET PRACTICE TIME (SECONDS):", True, WHITE)
        screen.blit(text1, (200, 200))

        input_box_rect = pygame.Rect(screen_width // 2 - 50, 300, 100, 50)
        pygame.draw.rect(screen, WHITE, input_box_rect, 2)
        time_surface = font3.render(practice_time, True, WHITE)
        screen.blit(time_surface, (screen_width // 2 - time_surface.get_width() // 2, 310))

        text_limit = font2.render("Limit: 3 digits", True, RED)
        screen.blit(text_limit, (screen_width // 2 - text_limit.get_width() // 2, 370))
    elif game_status == game:
        screen.fill(BLACK)
        text1 = font2.render("PRESS ESC TO QUIT", True, WHITE)
        text_time = font2.render(f"Time Remaining: {time_remaining} s", True, WHITE)
        screen.blit(text1, (15, 10))
        screen.blit(text_time, (15, 30))
        ...
        screen.fill(BLACK)
        text1 = font3.render(f"Correct Keys Pressed: {correct_keys_pressed}", True, WHITE)
        text2 = font3.render("Press R to restart", True, WHITE)
        text3 = font3.render("Press Q to quit", True, WHITE)
        screen.blit(text1, (280, 450))
        screen.blit(text2, (280, 500))
        screen.blit(text3, (280, 530))
